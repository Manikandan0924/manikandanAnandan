...
$query = "SELECT d.id, d.name, l.name as locationName FROM department d LEFT JOIN location l ON d.locationID = l.id ORDER BY d.name ASC";

// If search text is provided, add a WHERE clause to filter results
if (!empty($searchText)) {
    $searchText = $conn->real_escape_string($searchText);
    $query .= " WHERE d.name LIKE '%$searchText%' OR l.name LIKE '%$searchText%'";
}
